...
for chunkindex, chunk in enumerate(splitbins):
    ...
    canname = "c_bkgfit"+str(chunkindex)
    ...
    for binindex, bin in enumerate(chunk):
        ...
        data = rootfile.Get("dataHist_bin"+str(bin))
        f1 = rootfile.Get("model_bkg_f1_bin"+str(bin))
        f2 = rootfile.Get("model_bkg_f2_bin"+str(bin))
        f3 = rootfile.Get("model_bkg_f3_bin"+str(bin))
...
#            print(str(i)+": "+str(val))
        ...
        chitext.DrawLatexNDC(m2ptextx,0.74,"#chi^{2}/dof="+chi2dofstr)

        # write the 2p mass range
        m2plo = ws.datahist2d.GetXaxis().GetBinLowEdge(bin)
        m2phi = ws.datahist2d.GetXaxis().GetBinUpEdge(bin)
        m2ptext=ROOT.TLatex()
        m2ptext.SetTextFont(42)
        m2ptext.SetTextSize(0.055)
        m2ptext.DrawLatexNDC(m2ptextx,0.80,str(m2plo)+"<M(2p)<"+str(m2phi)+" GeV")
