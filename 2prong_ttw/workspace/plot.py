...
hchi2prob = ROOT.TH1D("hchi2prob"+ttw.wbin,"hchi2prob"+ttw.wbin,10,0.,1.)
...
for btagindex, btagbin in enumerate(ttw.btagbins):
    ...
    canlist = []
    canlist.append(ROOT.TCanvas("c1_"+ttw.wbin+"_"+btagbin, "c1_"+ttw.wbin+"_"+btagbin, 1000, 300))
    canlist.append(ROOT.TCanvas("c2_"+ttw.wbin+"_"+btagbin, "c2_"+ttw.wbin+"_"+btagbin, 1000, 300))
    ...
    for ptindex, ptbin in enumerate(ttw.ptbins):
        ...
        bkg2 = rootfile.Get("temp_"+btagbin+"_"+ptbin+"_pdf2")
        bkg3 = rootfile.Get("temp_"+btagbin+"_"+ptbin+"_pdf3")
        data.plotOn(plot,ROOT.RooFit.MarkerSize(0.5),ROOT.RooFit.Name("data"))
        if ttw.wbin=="asymnoniso":
            ...
            bkg3.plotOn(plot,ROOT.RooFit.LineColor(colors[3]),ROOT.RooFit.LineWidth(1),ROOT.RooFit.Name("bkg3"))
            fitresult = rootfile.Get("fitresult_temp_"+ttw.wbin+"_"+btagbin+"_"+ptbin+"_bkg3pdf_data_"+ttw.wbin+"_"+btagbin+"_"+ptbin)
            fitresult.Print("v")
            print("Cov. matrix")
...
#            txt.DrawLatexNDC(0.45,0.76,wtitles[windex])
        ...
        print(pttitles[ptindex]+" "+btagtitles[btagindex])
        if ttw.wbin=="asymnoniso":
...
can = ROOT.TCanvas("cprob"+ttw.wbin,"cprob"+ttw.wbin,500,500)
can.cd()
h=hchi2prob
h.Draw()
h.SetMinimum(0)
