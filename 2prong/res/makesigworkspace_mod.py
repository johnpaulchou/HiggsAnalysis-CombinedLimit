...
def main(argv=None):
    ...
    if args.raw and args.fixmasses is not None:
        parser.error("--raw cannot be used with --fixmasses")

    if args.raw and args.imass is None:
        parser.error("--raw requires --imass")
    ...
    rawFound = False
    if args.raw:
        # Raw mode: use the file directly, no interpolation
        fnRaw = files.genfilenames_raw[args.imass]
        #(windex,pindex) = files.indexpair(args.imass)
        #fnRaw = files.genfilenames[windex][pindex]
        #print("############")
        #print(windex, pindex)
        #print(files.genfilenames)
        #print(files.genfilenames[0][0])
